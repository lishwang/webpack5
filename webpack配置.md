...
### 手写 babel-loader，实现把es6+的语法转换成es5语法

参考 [babel官网](https://babeljs.io/docs/en/babel-core)

##### babel 核心库的介绍

- [@babel/parser](https://babeljs.io/docs/en/babel-parser) ：将js代码解析成抽象语法树AST；
- [@babel/traverse](https://babeljs.io/docs/en/babel-traverse) ：将抽象语法树AST格式化成一个更好操作的树状结构的对象；并通过依赖分析对文件各种处理，最终将文件合并成一个文件；
- [@babel/generator](https://babeljs.io/docs/en/babel-generator) ：将上面处理好的文件输出成一个正常的代码，并输出出去；

##### 本次只做简单的babel功能--把es6+的语法转换成es5语法

- 想把es6+的语法转换成es5语法，就需要用到官网中的 `@babel/preset-env` 这个预设，配置在webpack文件的loader的options配置中；
- 另外需要用到 babel 的一些核心库 `@babel/core` 对js代码进行编译转换；

##### 代码如下：

- loaders\test-loader\my-babel-loader\index.js
...
- loaders\test-loader\my-babel-loader\schema.json
